...
async function assembler1() {
  await assemblerPart(1000);
  await assemblerPart(2000);
  await assemblerPart(3000);
}
...
async function assembler2() {
  await getInformation().then(() => assemblerPart(1000));
  await getInformation().then(() => assemblerPart(1000));
  await getInformation().then(() => assemblerPart(1000));
}
...
async function assembler3() {
  await readManual().then(() =>
    downloadManual().then(() => assemblerPart(1000))
  );
  await readManual().then(() =>
    downloadManual().then(() => assemblerPart(1000))
  );
...
function assemblerParts() {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      let error = Math.random() > 0.4;
      if (error) {
        resolve(console.log("Parte armada"));
      } else {
        reject("Error al armar la parte");
      }
    }, 2000);
  });
}

async function assembler4() {
  try {
    await assemblerParts();
    await assemblerParts();
    await assemblerParts();
  } catch (error) {
    console.log(error);
